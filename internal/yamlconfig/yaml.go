...
func (m *Manager) Validate(ctx context.Context, data []byte) (models.ValidationResult, error) {
	...
	return m.validateDocument(doc), nil
}

func (m *Manager) validateDocument(doc models.ConfigYAMLDocument) models.ValidationResult {
	result := nodesbackup.Validate(models.NodesBackupDocument{
		Version: doc.Version,
		Nodes:   doc.Nodes,
	})
	result.Warnings = append(result.Warnings,
		"only the nodes section will be applied; users, keys, credentials, and settings are not changed")
	return result
}
